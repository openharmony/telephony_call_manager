...
private:
    ...
    static void ReportCallEventWork(uv_work_t *work, int status);
    static int32_t ReportCallEvent(CallEventInfo &info, EventListener stateCallback);
    ...
    static void ReportWaitAndLimitInfoWork(uv_work_t *work, int status);
    static void ReportWaitAndLimitInfo(AppExecFwk::PacMap &resultInfo, EventListener supplementInfo);
    static void ReportSupplementInfoWork(uv_work_t *work, int status);
    static void ReportSupplementInfo(AppExecFwk::PacMap &resultInfo, EventListener supplementInfo);
    static void ReportSettingInfoWork(uv_work_t *work, int status);
    static void ReportSettingInfo(EventListener &settingInfo, AppExecFwk::PacMap &resultInfo);
...
private:
    ...
    EventListener setVolteCallback_;
    EventListener getVolteCallback_;
    std::mutex mutex_;
};
} // namespace Telephony
} // namespace OHOS
