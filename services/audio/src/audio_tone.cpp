...
AudioTone::AudioTone(ToneDescriptor tone)
{
...
int32_t AudioTone::Play()
{
    ...
    std::thread play(AudioPlayer::Play, GetToneDescriptorPath(currentToneDescriptor_),
        AudioStreamType::STREAM_MUSIC, playerType);
    play.detach();
    return TELEPHONY_SUCCESS;
}

int32_t AudioTone::Stop()
{
    if (currentToneDescriptor_ == TONE_UNKNOWN) {
        TELEPHONY_LOGE("tone descriptor unknown");
        return TELEPHONY_ERR_FAIL;
...
ToneDescriptor AudioTone::ConvertDigitToTone(char digit)
{
...
bool AudioTone::IsDtmf(ToneDescriptor tone)
{
    bool ret = false;
    switch (tone) {
...
std::string AudioTone::GetToneDescriptorPath(ToneDescriptor tone)
{
