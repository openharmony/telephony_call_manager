...
int32_t SuperPrivacyManagerClient::SetSuperPrivacyPolicies(const SuperPrivacyPolicyInfo &info, int32_t source)
{
    ...
    if (!data.WriteParcelable(&info)) {
        TELEPHONY_LOGE("Write [superPrivacyPolicyInfo] failed!");
        return ERR_INVALID_DATA;
    }
    if (!data.WriteInt32(source)) {
        return ERR_INVALID_DATA;
    }
    int32_t result = sa->SendRequest(COMMAND_SET_SUPER_PRIVACY_POLICIES, data, reply, option);
    TELEPHONY_LOGI("SetSuperPrivacyPolicies, ret=%{public}d", result);
    return result;
}
} // namespace Telephony
} // namespace OHOS