...
void CallAbilityHandler::ProcessEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    ...
    TELEPHONY_LOGI("ProcessEvent inner event id obtained: %{public}u.", event->GetInnerEventId());
    switch (event->GetInnerEventId()) {
        ...
        case CallAbilityHandlerService::HANDLER_REPORT_CALL_EVENT: {
            ...
            DelayedSingleton<CallAbilityReportProxy>::GetInstance()->ReportCallEvent(info);
            break;
        }
        default: {
