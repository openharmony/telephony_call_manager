    ...
    : AppExecFwk::EventHandler(runner)
{
    ...
    memberFuncMap_[CellularCallInfoHandlerService::HANDLER_UPDATE_DISCONNECTED_CAUSE] =
        &CellularCallInfoHandler::ReportDisconnectedCause;
...
void CellularCallInfoHandler::ProcessEvent(const AppExecFwk::InnerEvent::Pointer &event)
{
    ...
    TELEPHONY_LOGI("CellularCallInfoHandler::ProcessEvent inner event id: %{public}u.", event->GetInnerEventId());
...
void CellularCallInfoHandler::ReportDisconnectedCause(const AppExecFwk::InnerEvent::Pointer &event)
{
...
int32_t CellularCallInfoHandlerService::UpdateDisconnectedCause(const DisconnectedDetails &cause)
{
    if (handler_.get() == nullptr) {
        TELEPHONY_LOGE("handler_ is nullptr");
        return TELEPHONY_ERR_FAIL;
