...
HWTEST_F(ZeroBranch10Test, Telephony_AudioProxy_002, TestSize.Level0)
{
    ...
    descs.clear();
    desc->modemCallSupported_ = false;
    descs.push_back(desc);
    std::make_shared<AudioPreferDeviceChangeCallback>()->OnPreferredOutputDeviceUpdated(descs);
    descs.clear();
...
/**
 * @tc.number   Telephony_IncomingFlashReminder_004
 * @tc.name     test IncomingFlashReminder
 * @tc.desc     Function test
 */
HWTEST_F(ZeroBranch10Test, Telephony_IncomingFlashReminder_004, TestSize.Level1)
{
    auto runner = AppExecFwk::EventRunner::Create("handler_incoming_flash_reminder");
    auto &reminder = DelayedSingleton<CallControlManager>::GetInstance()->incomingFlashReminder_;
    // 首先考虑读取句柄为空，然后执行几个函数看看。
    reminder = std::make_shared<IncomingFlashReminder>(runner, nullptr);
    reminder->libAdapterHandler_ = nullptr;

    bool isNecessary = reminder->IsFlashRemindNecessary();
    EXPECT_EQ(isNecessary, false);
    reminder->HandleStartFlashRemind();
    EXPECT_EQ(reminder->isFlashRemindUsed_, true);
    EXPECT_EQ(reminder->libAdapterHandler_, nullptr);
    reminder->IsTorchReady();
    reminder->IsScreenStatusSatisfied();
    reminder->HandleSetTorchMode();
    reminder->HandleStopFlashRemind();
    reminder->HandleEndFlashRemind();
...
/**
 * @tc.number   Telephony_IncomingFlashReminder_005
 * @tc.name     test IncomingFlashReminder
 * @tc.desc     Function test
 */
HWTEST_F(ZeroBranch10Test, Telephony_IncomingFlashReminder_005, TestSize.Level1)
{
    auto runner = AppExecFwk::EventRunner::Create("handler_incoming_flash_reminder");
    auto &reminder = DelayedSingleton<CallControlManager>::GetInstance()->incomingFlashReminder_;
    // dlopen成功但是dlsym失败
    ...
    reminder->HandleStartFlashRemind();
    EXPECT_EQ(reminder->isFlashRemindUsed_, true);
    reminder->libAdapterHandler_ = dlopen("libtel_call_manager_api.z.so", RTLD_LAZY);
    EXPECT_NE(reminder->libAdapterHandler_, nullptr);
    reminder->IsTorchReady();
    reminder->IsScreenStatusSatisfied();
    reminder->HandleSetTorchMode();
    reminder->HandleStopFlashRemind();
...
/**
 * @tc.number   Telephony_IncomingFlashReminder_006
 * @tc.name     test IncomingFlashReminder
 * @tc.desc     Function test
 */
HWTEST_F(ZeroBranch10Test, Telephony_IncomingFlashReminder_006, TestSize.Level1)
{
    auto runner = AppExecFwk::EventRunner::Create("handler_incoming_flash_reminder");
    auto &reminder = DelayedSingleton<CallControlManager>::GetInstance()->incomingFlashReminder_;
    // 正常dlopen并且读取数据。
    ...
    reminder->libAdapterHandler_ = dlopen("libtel_cm_deps_adapter.z.so", RTLD_LAZY);
    EXPECT_NE(reminder->libAdapterHandler_, nullptr);
    bool isTorchOk = false;
    EXPECT_EQ(reminder->IsTorchReady(), isTorchOk);
    reminder->IsScreenStatusSatisfied();
