 ...
 * @tc.name     test CallControlManager HangUpOtherCall with HasBtCallWithDifferentNumber
 * @tc.desc     Function test - cover HasBtCallWithDifferentNumber() == true branch
 */
HWTEST_F(ZeroBranch4Test, Telephony_CallControlManager_015, Function | MediumTest | Level1)
{
    ...
    std::shared_ptr<CallControlManager> callControlManager = std::make_shared<CallControlManager>();
    DialParaInfo btPara;
    sptr<CallBase> btCall = new BluetoothCall(btPara, "AA:BB:CC:DD:EE:FF");
    btCall->callId_ = 0;
    btCall->SetCallIndex(0);
    btCall->SetSlotId(0);
    btCall->SetTelCallState(TelCallState::CALL_STATUS_ACTIVE);
    btCall->SetCallType(CallType::TYPE_BLUETOOTH);
    btCall->SetAccountNumber("13800138000");
    CallObjectManager::callObjectPtrList_.push_back(btCall);
    DialParaInfo imsPara;
    sptr<CallBase> imsCall = new IMSCall(imsPara);
    imsCall->callId_ = 1;
    imsCall->SetCallIndex(1);
    imsCall->SetSlotId(0);
    imsCall->SetTelCallState(TelCallState::CALL_STATUS_ACTIVE);
    imsCall->SetCallType(CallType::TYPE_IMS);
    imsCall->SetAccountNumber("13900139000");
    CallObjectManager::callObjectPtrList_.push_back(imsCall);
    DialParaInfo incomingPara;
    sptr<CallBase> incomingCall = new IMSCall(incomingPara);
    incomingCall->callId_ = 2;
    incomingCall->SetCallIndex(2);
    incomingCall->SetSlotId(0);
    incomingCall->SetTelCallState(TelCallState::CALL_STATUS_INCOMING);
    incomingCall->SetCallType(CallType::TYPE_IMS);
    incomingCall->SetAccountNumber("13900139000");
    CallObjectManager::callObjectPtrList_.push_back(incomingCall);
    EXPECT_TRUE(callControlManager->HangUpOtherCall(2));
    CallObjectManager::callObjectPtrList_.clear();
#endif
}
