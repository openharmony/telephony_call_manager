...
HWTEST_F(ZeroBranch7Test, Telephony_CallWiredHeadSet_001, Function | MediumTest | Level1)
{
    ...
    instance->DealKeyPressedDown(nullptr);
    instance->DealKeyPressedUp(nullptr);
...
/**
 * @tc.number   Telephony_CallBroadCastSubscriber_005
 * @tc.name     test OnReceiveEvent ex
 * @tc.desc     Function test
 */
HWTEST_F(ZeroBranch7Test, Telephony_CallBroadCastSubscriber_005, Function | MediumTest | Level1)
{
    g_receiveUnknownEvent = false;
    EventFwk::MatchingSkills matchingSkills;
    EventFwk::CommonEventSubscribeInfo subscriberInfo(matchingSkills);
...
HWTEST_F(ZeroBranch7Test, Telephony_ThermalProtection_001, Function | MediumTest | Level1)
{
    ...
    CallAttributeInfo info1;
    info1.isEcc = true;
    call1->GetCallAttributeInfo(info1);
    callControl->HandleThermalLevelChange(4);
...
/**
 * @tc.number   Telephony_IMSCall_UpdateImsRttCallMode_Success
 * @tc.name     test UpdateImsRttCallMode returns TELEPHONY_SUCCESS
 * @tc.desc     Function test - cover UpdateImsRttCallMode with SUCCESS path
 */
HWTEST_F(ZeroBranch7Test, Telephony_IMSCall_UpdateImsRttCallMode_Success, TestSize.Level0)
{
#ifdef SUPPORT_RTT_CALL
    DialParaInfo info;
    std::shared_ptr<IMSCall> call = std::make_shared<IMSCall>(info);
    ...
    EXPECT_GT(call->UpdateImsRttCallMode(ImsRTTCallMode::REMOTE_REQUEST_UPGRADE_LOCAL_ACCEPT), TELEPHONY_ERROR);
    EXPECT_EQ(call->GetRttState(), RttCallState::RTT_STATE_YES);
#endif
}
 
/**
 * @tc.number   Telephony_IMSCall_UpdateImsRttCallMode_LocalReject
 * @tc.name     test UpdateImsRttCallMode with LOCAL_REQUEST_DOWNGRADE
 * @tc.desc     Function test - cover rttState_ set to RTT_STATE_NO for LOCAL_REQUEST_DOWNGRADE
 */
HWTEST_F(ZeroBranch7Test, Telephony_IMSCall_UpdateImsRttCallMode_LocalReject, TestSize.Level0)
{
#ifdef SUPPORT_RTT_CALL
    DialParaInfo info;
    std::shared_ptr<IMSCall> call = std::make_shared<IMSCall>(info);
    call->SetCallId(1);
    call->accountNumber_ = "123456";
    EXPECT_GT(call->UpdateImsRttCallMode(ImsRTTCallMode::REMOTE_REQUEST_UPGRADE_LOCAL_REJECT), TELEPHONY_ERROR);
    EXPECT_EQ(call->GetRttState(), RttCallState::RTT_STATE_NO);
#endif
}
